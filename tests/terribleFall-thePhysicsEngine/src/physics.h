...
public:
  ...
  float getY();
  /**
   * Apply a force of the sent magnitude to the thing
...
public:
   ...
   * @param thing - a pointer to a thing (e.g. &thing)
   */
  void addThing(Thing* thing);
  /**
   * Checks if the two things collided,
   * true if distance < radius1 + radius2
   * @param thing1 - a pointer to a thing (e.g. &thing)
   * @param thing2 - a pointer to a thing (e.g. &thing)
   * @return bool
   */
